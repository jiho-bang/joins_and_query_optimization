...
public class QueryPlan {
    ...
    public QueryOperator minCostSingleAccess(String table) {
        ...
        int c = -1;
        int io = minOp.estimateIOCost();
        for (int i : getEligibleIndexColumns(table)) {
            SelectPredicate p = selectPredicates.get(i);
            QueryOperator q = new IndexScanOperator(transaction, table, p.column, p.operator, p.value);
            int ind = q.estimateIOCost();
            if (io > ind) {
                c = i;
                minOp = q;
                io = ind;
        ...
        minOp = addEligibleSelections(minOp, c);
        return minOp;
    }
    ...
    public Map<Set<String>, QueryOperator> minCostJoins(
        ...
        for (Set<String> k : prevMap.keySet()) {
            for (JoinPredicate p : joinPredicates) {
                QueryOperator t;
                QueryOperator t2;
                Set<String> keys = new HashSet<>(k);
                Set<String> set = new HashSet<>();
                if (k.contains(p.leftTable) && !k.contains(p.rightTable)) {
                    set.add(p.rightTable);
                    t = pass1Map.get(set);
                    t2 = minCostJoinType(prevMap.get(k), t, p.leftColumn, p.rightColumn);
                    keys.add(p.rightTable);
                } else if (!k.contains(p.leftTable) && k.contains(p.rightTable)) {
                    set.add(p.leftTable);
                    t = pass1Map.get(set);
                    t2 = minCostJoinType(prevMap.get(k), t, p.rightColumn, p.leftColumn);
                    keys.add(p.leftTable);
                } else {
                    continue;
                }

                if (result.containsKey(keys)) {
                    if (result.get(keys).estimateIOCost() > t2.estimateIOCost())
                        result.put(keys, t2);
                } else {
                    result.put(keys, t2);
                }
    ...
    public Iterator<Record> execute() {
        ...
        int s = tableNames.size();
        Map<Set<String>, QueryOperator> minCost = new HashMap<>();
        for (String t : tableNames) {
            Set<String> map = new HashSet<>();
            map.add(t);
            minCost.put(map, minCostSingleAccess(t));
        }

        Map<Set<String>, QueryOperator> t2 = minCost;
        for (int i = 2; i <= s; i++) {
            t2 = minCostJoins(t2, minCost);
        }

        finalOperator = minCostOperator(t2);
        addGroupBy();
        addProject();
        addLimit();
