...
public class GHJOperator extends JoinOperator {
    ...
    private void partition(Partition[] partitions, Iterable<Record> records, boolean left, int pass) {
        ...
        for (Record r : records) {
            DataBox col;
            if (left) {
                col = r.getValue(getLeftColumnIndex());
            } else {
                col = r.getValue(getRightColumnIndex());
            }

            int hash;
            int mod;
            hash = HashFunc.hashDataBox(col, pass);
            mod = hash % partitions.length;
            if (0 > mod) {
                mod += partitions.length;
            }
            partitions[mod].add(r);
        }
        return;
    }
    ...
    private void buildAndProbe(Partition leftPartition, Partition rightPartition) {
        ...
        Map<DataBox, List<Record>> hMap = new HashMap<>();

        for (Record b : buildRecords) {
            DataBox bCol = b.getValue(buildColumnIndex);
            if (!hMap.containsKey(bCol)) {
                hMap.put(bCol, new ArrayList<>());
            }
            hMap.get(bCol).add(b);
        }
        for (Record p : probeRecords) {
            DataBox pCol = p.getValue(probeColumnIndex);
            if (!hMap.containsKey(pCol)) {
                continue;
            }

            for (Record b2 : hMap.get(pCol)) {
                Record g;
                if (probeFirst) {
                    g = p.concat(b2);
                } else {
