...
public class SortOperator extends QueryOperator {
    ...
    public Run sortRun(Iterator<Record> records) {
        ...
        Run order = new Run(transaction, getSchema());

        while (records.hasNext()) {
            rList.add(records.next());
            Collections.sort(rList, new RecordComparator());
        }
    ...
    public Run mergeSortedRuns(List<Run> runs) {
        ...
        for (Run r : runs) {
            merge.add(r.iterator());
        }
        for (int i = 0; i < merge.size(); i++) {
            if (merge.get(i).hasNext()) {
                pq.add(new Pair<>(merge.get(i).next(), i));
            }
        }
    ...
    public List<Run> mergePass(List<Run> runs) {
        ...
        for (int i = 0; i + numBuffers - 1 <= runs.size() + 1; i += numBuffers - 1) {
            List<Run> add = runs.subList(i, Math.min(runs.size(), numBuffers + i - 1));
            run.add(mergeSortedRuns(add));
        }
        return run;
    }
    ...
    public Run sort() {
        ...
        // TODO(proj3_part1): implement
        List<Run> mList = new ArrayList<>();
        while (sourceIterator.hasNext()) {
            Run merge = sortRun(getBlockIterator(sourceIterator, getSchema(), numBuffers));
            mList.add(merge);
        }

        for (int i = 0; i < mList.size() - 1; i++) {
            mList = mergePass(mList);
        }
        return mList.get(0); // TODO(proj3_part1): replace this!
    }
